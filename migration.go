...
func (m *Migration) run(db *sql.DB, direction bool) error {
	...
	case ".sql":
		...
		if err := runSQLMigration(db, m, statements, useTx, m.Version, direction); err != nil {
	...
	case ".go":
		if !m.Registered {
			return errors.Errorf("ERROR %v: failed to run Go migration: Go functions must be registered and built into a custom binary (see https://github.com/kidgilson/goose/tree/master/examples/go-migrations)", m.Source)
		}
		tx, err := db.Begin()
		if err != nil {
